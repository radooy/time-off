...
function Request() {
  ...
  const submitRequest = () => {
    ...
    const paidLeave = isPaidLeaveRequest
      ? Number(Number(daysLeft) - diffInBusDays)
      : daysLeft;
  ...
  const disableWeekends = (date) => {
    return (
      date.getDay() === constants.sundayCalendarNumber ||
      date.getDay() === constants.saturdayCalendarNumber
    );
  };

  const sumTotalDaysOutOfOffice = useCallback(() => {
    const sum = differenceInCalendarDays(
      addBusinessDays(startDate, diffInBusDays),
      startDate
    );

    return startDate.getDay() === constants.mondayCalendarNumber
      ? sum + constants.weekendValue
      : sum;
  ...
  return (
    <StyledContainer>
      ...
      <FormControl fullWidth>
        ...
        >
          <MenuItem value={constants.paidLeaveValue}>
            {constants.paidLeave}
          </MenuItem>
          <MenuItem value={constants.sickLeaveValue}>
            {constants.sickLeave}
          </MenuItem>
        </Select>
      </FormControl>
      ...
      {error !== "" && <StyledError className="pl-10">{error}</StyledError>}
